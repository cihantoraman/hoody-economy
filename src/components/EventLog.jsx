...
const EventCard = ({ event, turnCount }) => {
  ...
  return (
    <div
      className="rounded-lg border border-line bg-surface-2 overflow-hidden transition-opacity duration-700"
      style={{ opacity: remaining < 30 ? 0.6 : 1 }}
    >
      <div className="h-1.5 bg-line">
        <div
          className={`h-full ${negative ? 'bg-danger' : 'bg-accent'} transition-[width] duration-700 ease-linear`}
