...
const stepEvents = (state, params, log) => {
  ...
  if (params.autoEvents && Math.random() < params.eventChance) {
    const base = MARKET_EVENTS[randInt(MARKET_EVENTS.length)];
    const event = {
      ...base,
      duration: base.duration + 3, // Longer events linger and fade rather than blink in and out.
...
const stepPrices = (products, historical, params, turn, log) => {
  ...
  const next = products.map((product) => {
    ...
    samples.push({ name: product.name, price });
...
export const simulateTurn = (state, params) => {
  ...
  historical = recordHistory(historical, players, finalized.mobility, turn);
